...
class TestDetection:
    ...
    def test_detects_nothing(self, temp_dir):
        assert detect_project_type(temp_dir) is None

    def test_role_wins_over_collection_layout(self, temp_dir):
    ...
    def test_find_collection_roles_sorted(self):
        roles = find_collection_roles(FIXTURE)
        assert list(roles.keys()) == ["first", "second"]
        assert roles["first"] == FIXTURE / "roles" / "first"

    def test_find_collection_roles_ignores_roles_without_specs(self, temp_dir):
...
class TestCollectionValidation:
    ...
    def test_orphaned_role_marker_warns(self, temp_dir):
        import shutil
    ...
    def test_mismatched_named_marker_is_error(self, temp_dir):
        import shutil
    ...
    def test_clean_fixture_validates_without_collection_findings(self):
