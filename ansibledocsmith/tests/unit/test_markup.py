...
class TestConvertAnsibleMarkupMarkdown:
    ...
    def test_rst_ref_renders_as_plain_text(self) -> None:
        assert (
    ...
    def test_module_link_for_wellformed_fqcn(self) -> None:
    ...
    def test_malformed_module_stays_verbatim(self) -> None:
    ...
    def test_plugin_link(self) -> None:
    ...
    def test_option_links_to_known_role_variable(self) -> None:
        result = convert_ansible_markup(
    ...
    def test_option_with_value_links_too(self) -> None:
        result = convert_ansible_markup(
    ...
    def test_unknown_option_renders_as_code(self) -> None:
        result = convert_ansible_markup(
            "See O(other_var).", "markdown", role_options={"foo_domain"}
        )
        assert result == "See `other_var`."

    def test_dotted_option_path_is_not_linked(self) -> None:
    ...
    def test_no_role_options_means_no_links(self) -> None:
...
class TestConvertAnsibleMarkupRST:
    ...
    def test_option_is_plain_literal_without_anchor_link(self) -> None:
        result = convert_ansible_markup(
            "See O(foo_domain).", "rst", role_options={"foo_domain"}
        )
        assert result == "See ``foo_domain``."

    def test_module_link(self) -> None:
...
class TestConvertAnsibleMarkupPassthrough:
    ...
    def test_byte_identical_without_markup(self, text: str) -> None:
        assert convert_ansible_markup(text, "markdown") == text
        assert convert_ansible_markup(text, "rst") == text

    def test_paragraph_separators_preserved_around_markup(self) -> None:
        text = "First C(one).\n\n\nSecond C(two)."
        result = convert_ansible_markup(text, "markdown")
        assert result == "First `one`.\n\n\nSecond `two`."

    def test_code_fences_with_markup_like_text_stay_untouched(self) -> None:
        fence = "```yaml\nfoo: C(not converted)\n```"
        assert convert_ansible_markup(fence, "markdown") == fence

    def test_indented_code_block_stays_untouched(self) -> None:
        block = "    foo = C(bar)\n    baz = 1"
        assert convert_ansible_markup(block, "markdown") == block

    def test_mixed_prose_and_fence_chunks(self) -> None:
...
class TestMarkupInGeneratorFilters:
    ...
    def test_table_truncation_never_splits_code_spans(self) -> None:
        generator = MarkdownDocumentationGenerator()
    ...
    def test_defaults_comments_use_markdown_without_anchor_links(self) -> None:
...
class TestLintAnsibleMarkup:
    ...
    def test_valid_markup_produces_no_messages(self) -> None:
        assert lint_ansible_markup("Use C(code) and M(ansible.builtin.copy).") == []

    def test_plain_text_produces_no_messages(self) -> None:
        assert lint_ansible_markup("No markup at all, just text.") == []
        assert lint_ansible_markup("") == []

    def test_code_blocks_are_not_linted(self) -> None:
        text = "```yaml\nvalue: M(bad)\n```"
